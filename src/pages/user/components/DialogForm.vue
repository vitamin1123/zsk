    ...
    <template #body>
      ...
      <t-form ref="form" :data="formData" :rules="rules" :label-width="100" @submit="onSubmit">
        ...
        <t-form-item style="float: right">
          <t-popconfirm content="确认删除吗" @confirm="onClickDelBtn(formData.name)">
...
import { delDocById, getCataList } from '@/api/list';
...
const onClickDelBtn = async (filename: string) => {
  console.log(filename);
  await delDocById(filename.replace(/<\/?[^>]+(>|$)/g, ''));
